...
class Login {
    ...
    public function CheckLogin() {
        if (empty($_SESSION['userlogin']) || $_SESSION['userlogin']['us_level'] < $this->Level):
    ...
    private function setLogin() {
        ...
        elseif (!$this->getUser()):
            ...
            $this->Result = false;
        elseif ($this->Result['us_level'] < $this->Level):
            $this->Error = ["Desculpe {$this->Result['us_nome']}, você não tem permissão para acessar esta área!", WS_ERROR];
            $this->Result = false;
        else:
            $this->Execute();
        endif;
    ...
    private function getUser() {
        ...
        $read = new Read;
        $read->ExeRead("usuarios", "WHERE us_email = :e AND us_senha = :p", "e={$this->Email}&p={$this->Senha}");
    ...
    private function Execute() {
        ...
        $this->Error = ["Olá {$this->Result['us_nome']}, seja bem vindo(a). Aguarde redirecionamento!", WS_ACCEPT];
        $this->Result = true;
    }
