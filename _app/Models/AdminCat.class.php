...
class AdminCat {
    ...
    public function ExeDelete($CategoryId) {
        ...
        $read = new Read;
        $read->ExeRead(self::Entity, "WHERE ct_idcategoria = :delid", "delid={$this->CatId}");
        ...
        if (!$read->getResult()):
            ...
            elseif ($category_parent && !$this->checkPosts()):
                ...
                $delete = new Delete;
                $delete->ExeDelete(self::Entity, "WHERE ct_idcategoria = :deletaid", "deletaid={$this->CatId}");
    ...
    private function setData() {
        ...
        $this->Data['categoria'] = Check::Name($this->Data['categoria']);
    }

    //Verifica o NAME da categoria. Se existir adiciona um pós-fix +1
    private function setName() {
        $Where = (!empty($this->CatId) ? "idCat != {$this->CatId} AND" : '' );

        $readName = new Read;
        $readName->ExeRead(self::Entity, "WHERE {$Where} ct_categoria = :t", "t={$this->Data['categoria']}");
    ...
    private function Create() {
        ...
        if ($Create->getResult()):
            ...
            $this->Error = ["<b>Sucesso:</b> A categoria {$this->Data['categoria']} foi cadastrada no sistema!", WS_ACCEPT];
        endif;
    }
    ...
    private function Update() {
        $Update = new Update;
        $Update->ExeUpdate(self::Entity, $this->Data, "WHERE ct_idcategoria = :catid", "catid={$this->CatId}");
        if ($Update->getResult()):
