...
class AdminUser {
    ...
    public function ExeDelete($UserId) {
        ...
        $readUser = new Read;
        $readUser->ExeRead(self::Entity, "WHERE us_idUser = :id", "id={$this->User}");
        ...
        elseif ($this->User == $_SESSION['userlogin']['user_id']):
            ...
            if ($readUser->getResult()[0]['us_level'] == 3):

                $readAdmin = $readUser;
                $readAdmin->ExeRead(self::Entity, "WHERE us_idUser != :id AND us_level = :lv", "id={$this->User}&lv=3");
    ...
    private function checkEmail() {
        ...
        $readUser = new Read;
        $readUser->ExeRead(self::Entity, "WHERE {$Where} us_email = :email", "email={$this->Data['user_email']}");
    ...
    private function Update() {
        ...
        $Update->ExeUpdate(self::Entity, $this->Data, "WHERE us_idUser = :id", "id={$this->User}");
        if ($Update->getResult()):
    ...
    private function Delete() {
        $Delete = new Delete;
        $Delete->ExeDelete(self::Entity, "WHERE us_idUser = :id", "id={$this->User}");
        if ($Delete->getResult()):
