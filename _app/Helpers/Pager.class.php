...
class Pager {
    ...
    private function getSyntax() {
        ...
        if ($this->Rows > $this->Limit):
            ...
            $this->Paginator .= "<li class=\"active\"><span class=\"active\">{$this->Page}</span></li>";

            for ($dPag = $this->Page + 1; $dPag <= $this->Page + $MaxLinks; $dPag ++):
                if ($dPag <= $Paginas):
                    $this->Paginator .= "<li><a title=\"Página {$dPag}\" href=\"{$this->Link}{$dPag}\">{$dPag} <span class=\"sr-only\">(current)</span></a></li>";
                endif;
            endfor;

            $this->Paginator .= "<li><a title=\"{$this->Last}\" href=\"{$this->Link}{$Paginas}\">{$this->Last}<span class=\"sr-only\">(current)</span></a></li>";
            $this->Paginator .= "</ul>";
        endif;
    }
